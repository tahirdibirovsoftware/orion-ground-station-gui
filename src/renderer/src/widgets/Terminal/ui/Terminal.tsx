...
const Terminal: FC<ITerminal> = ({ mode = 'demo', flightData }): JSX.Element => {
    ...
    const dataTypes = [
        'PACKET NUMBER', 'SATELLITE STATUS', 'ERROR CODE', 'MISSION TIME (DD/MM/YYYY/HH/MM/SS)', 'PRESSURE1 (hPa)',
        'PRESSURE2 (hPa)', 'ALTITUDE1 (m)', 'ALTITUDE2 (m)', 'ALTITUDE DIFFERENCE (m)', 'DESCENT RATE (m/s)', 'TEMP (°C)',
        'BATTERY V', 'GPS1 LATITUDE', 'GPS1 LONGITUDE', 'GPS1 ALTITUDE', 'PITCH°', 'ROLL°', 'YAW°', 'LNLN', 'IoT DATA (°C)',
        'TEAM NUMBER'
    ];
                ...
                <thead>
                    <tr>
                        {dataTypes.map(dataType => (
                            <th key={dataType} style={thLocaleStyle}>{dataType}</th>
                        ))}
                    </tr>
                </thead>
                <tbody ref={tableBodyRef}>
                    {flightData[flightData?.length - 1].packetNumber > 0 && filterToLastData<ITelemetry>(flightData.slice(1), 100).map(data => (
